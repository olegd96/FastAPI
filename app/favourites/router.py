...
async def check_fav(
    ...
    res = await FavDao.check_fav_hotel_room(
        room_id=room.id, hotel_id=room.h_id, user_id=user.id
    )
    if not res:
...
@router.get("")
async def get_all_fav(user: Users = Depends(get_current_user)) -> list[SFav]:
...
async def check_fav_anon(
    ...
    res = await FavDao.check_fav_hotel_room(
        room_id=room.id, hotel_id=room.h_id, anonimous_id=anonimous_id
    )
    if not res:
        raise CannotCheckFav
    return res
