...
async def s3_1(im_path: Path):
        await s3_client.upload_file(f"app/static/images/resized_500_300_{im_path.name}")

async def s3_2(im_path: Path):
    await s3_client.upload_file(f"app/static/images/resized_200_100_{im_path.name}")
